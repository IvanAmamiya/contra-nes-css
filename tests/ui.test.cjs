...
function boot(expanded=false,storage=new Map()){
  ...
  const buttons=['left','right','up','down','jump','fire','turbo','swap','bomb','lock','spin'].map(name=>{const el=get(name);el.dataset.control=name;return el;});
...
test('暂停时R重开，分数、生命和武器一并重置为所选30命',()=>{const a=boot();a.start();a.world.score=900;a.world.lives=1;a.key('KeyP');a.key('KeyR');assert.equal(a.world.state,'playing');assert.equal(a.world.score,0);assert.equal(a.world.lives,30);});
...
test('标题画面Konami代码仍可开启30命，菜单也可选择3命',()=>{const a=boot();for(const code of ['ArrowUp','ArrowUp','ArrowDown','ArrowDown','ArrowLeft','ArrowRight','ArrowLeft','ArrowRight','KeyB','KeyA'])a.key(code);a.start();assert.equal(a.world.lives,30);const b=boot();b.get('lives-select').value='3';b.get('lives-select').dispatch('change');b.start();assert.equal(b.world.lives,3);});
