...
#include <string.h>
#include "libretro.h"
static unsigned frame,pixel_format; static uint16_t keys;
...
int main(int argc,char**argv){if(argc<2)return 2;HMODULE lib=LoadLibraryA("snes9x_libretro.dll");if(!lib){printf("dll error %lu",GetLastError());return 3;}
 ...
 FILE*trace=fopen("trace.csv","w");fprintf(trace,"frame,flags,timer,lives,x,y,stage\n");
 for(frame=0;frame<3000;frame++){
  ...
  if(frame>=600&&frame%60==0){char name[50];sprintf(name,"vram-%u.bin",frame);dump(name,retro_get_memory_data(RETRO_MEMORY_VIDEO_RAM),retro_get_memory_size(RETRO_MEMORY_VIDEO_RAM));size_t s=retro_serialize_size();void*b=malloc(s);if(retro_serialize(b,s)){sprintf(name,"state-%u.bin",frame);dump(name,b,s);}free(b);}
 }fclose(trace);retro_deinit();free(r);return 0;}
