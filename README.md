...
更新：城市模式接入 ROM 的 28 屏地图、人物动作、碰撞数据与实测移动/射击规则。130 帧移动坐标完全匹配，253 项自动测试通过。具体完成范围与尚未重建的机制见 [首关重建记录](ROM_CITY.md)；这仍是重建进度版。
...
NES 首关稳定版本保留在标签 `v1.2.2`。`main` 发布 v2.0.0 精神 FC 扩展，`feature/sfc-contra` 保留同一扩展实现以便后续开发。切换前先保存正在进行的改动，再执行 `git switch feature/sfc-contra`。
